...
from prev.SparseTrainDenseTest import SparseTrainDenseTest
import numpy as np
...
streams = os.listdir('real_streams')
try:
...
for str_id, str_name in enumerate(streams):
    for _training_int_id, _training_int in enumerate(training_intervals):
        ...
        stream = ARFFParser('real_streams/%s' % str_name, chunk_size=chunk_size, n_chunks=n_chunks)
