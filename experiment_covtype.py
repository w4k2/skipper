...
# !
# TODO ova/ovo for replications? and then wilcoxon
...
deltas = [1, 10, 20, 60]
n_epochs = [1, 50, 50, 50]
results = np.zeros((4, 4, n_chunks, 3))
...
for d_id, d in enumerate(deltas):
    ...
    for f_id in range(4):
        ...
        results[d_id, f_id, 1:, 0] = f.scores
        results[d_id, f_id, :len(f.label_request_chunks), 1] = f.label_request_chunks
        results[d_id, f_id, :len(f.training_chunks), 2] = f.training_chunks

        print(f.label_request_chunks)
        print(f.training_chunks)
        np.save('results/res_covtype.npy', results)